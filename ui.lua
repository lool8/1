...
function aa.New(af)
    ...
    -- 创建独立 GUI 存放悬浮按钮
    local ah = ac("ScreenGui", {
        Name = "WindUI_SquareButton",
    ...
    local ai = ac("TextButton", {
        ...
        Text = (af.Title and af.Title:sub(1,1)) or "M",
        TextColor3 = Color3.new(1, 1, 1),
        TextSize = 24,
    ...
    shadow.ZIndex = ai.ZIndex - 1
    
    -- 悬停效果
    ai.MouseEnter:Connect(function()
        ad(ai, 0.1, { BackgroundColor3 = Color3.fromRGB(105, 118, 255) }):Play()
        ad(ai, 0.1, { Size = UDim2.new(0, 52, 0, 52) }):Play()
    end)
    ai.MouseLeave:Connect(function()
        ad(ai, 0.1, { BackgroundColor3 = Color3.fromRGB(88, 101, 242) }):Play()
        ad(ai, 0.1, { Size = UDim2.new(0, 50, 0, 50) }):Play()
    end)
    ...
    ai.InputBegan:Connect(function(input)
    ...
    ae.InputChanged:Connect(function(input)
    ...
    ae.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = false
        end
    end)
    ...
    ai.MouseButton1Click:Connect(function()
        if af.Parent and af.Parent.Visible then
            af.Parent.Visible = false
            ai.BackgroundColor3 = Color3.fromRGB(60, 70, 180)
        else
            if af.Parent then
                af.Parent.Visible = true
                ai.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
            end
        end
    end)
    ...
    function ag.Edit(ao, ap)
        if ap.Title then
            ai.Text = ap.Title:sub(1, 1) or "M"
        end
        if ap.Position then
            ai.Position = ap.Position
        end
    end
